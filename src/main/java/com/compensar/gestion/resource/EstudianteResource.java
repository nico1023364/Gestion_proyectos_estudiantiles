...
import com.compensar.gestion.service.EstudianteService;
...
public class EstudianteResource {
    ...
    public Response crear(Estudiante estudiante) {
        try {
            return Response.status(Response.Status.CREATED).entity(service.crear(estudiante)).build();
        } catch (BusinessException e) {
    ...
    public Response actualizar(@PathParam("id") Long id, Estudiante estudiante) {
        try {
            return Response.ok(service.actualizar(id, estudiante)).build();
        } catch (BusinessException e) {
    ...
    public Response eliminar(@PathParam("id") Long id) {
        try {
            service.eliminar(id);
            return Response.ok(Map.of("mensaje", "Estudiante eliminado correctamente.")).build();
        } catch (BusinessException e) {
            Response.Status estado = e.getMessage().contains("No existe") ? Response.Status.NOT_FOUND : Response.Status.BAD_REQUEST;
            return error(estado, e.getMessage());
        }
