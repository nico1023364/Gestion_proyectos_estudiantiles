...
import com.compensar.gestion.service.ProyectoService;
...
public class ProyectoResource {
    ...
    public Response crear(Proyecto proyecto) {
        try {
            return Response.status(Response.Status.CREATED).entity(service.crear(proyecto)).build();
        } catch (BusinessException e) {
    ...
    @PUT
    @Path("/{id}")
    public Response actualizar(@PathParam("id") Long id, Proyecto proyecto) {
        try {
            return Response.ok(service.actualizar(id, proyecto)).build();
        } catch (BusinessException e) {
    ...
    public Response eliminar(@PathParam("id") Long id) {
        try {
            service.eliminar(id);
            return Response.ok(Map.of("mensaje", "Proyecto eliminado correctamente.")).build();
        } catch (BusinessException e) {
            Response.Status estado = e.getMessage().contains("No existe") ? Response.Status.NOT_FOUND : Response.Status.BAD_REQUEST;
            return error(estado, e.getMessage());
        }
