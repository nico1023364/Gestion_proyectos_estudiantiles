...
import com.compensar.gestion.service.DocenteService;
...
public class DocenteResource {
    ...
    public Response crear(Docente docente) {
        try {
            return Response.status(Response.Status.CREATED).entity(service.crear(docente)).build();
        } catch (BusinessException e) {
    ...
    @PUT
    @Path("/{id}")
    public Response actualizar(@PathParam("id") Long id, Docente docente) {
        try {
            return Response.ok(service.actualizar(id, docente)).build();
        } catch (BusinessException e) {
    ...
    public Response eliminar(@PathParam("id") Long id) {
        try {
            service.eliminar(id);
            return Response.ok(Map.of("mensaje", "Docente eliminado correctamente.")).build();
        } catch (BusinessException e) {
            Response.Status estado = e.getMessage().contains("No existe") ? Response.Status.NOT_FOUND : Response.Status.BAD_REQUEST;
            return error(estado, e.getMessage());
        }
